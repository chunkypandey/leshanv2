    ...
    .directive('instance', function ($compile, $routeParams, $http, dialog, $filter, lwResources, $modal, helper, $rootScope) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.openExcecuteWithParamsModal = function (resourcePath) {
                    scope.resourcePath = resourcePath;
                    scope.excecuteWithParams = $modal.open({
                        templateUrl: 'partials/excecuteWithParams-modal.html',
                        scope: scope
                    });
                };

                scope.closeExcecuteWithParamsModal = function () {
                    scope.excecuteWithParams.close();
                };

                scope.execWithParams = function (pmin, pmax, lt, gt, st) {

                    alert("execute with params");
                    // scope.pmin = pmin;
                    // scope.pmax = pmax;
                    // scope.lt = lt;
                    // scope.gt = gt;
